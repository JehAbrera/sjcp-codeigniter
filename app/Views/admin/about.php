<main class=" flex flex-[4.5] flex-col p-4 bg-zinc-100 max-h-screen overflow-auto text-slate-950 gap-4">
    ...
    <div class=" flex justify-between items-center">
        <span class=" badge badge-success badge-outline p-4 font-bold text-2xl">Content List</span>
        <form action="/admin/about/add" id="addform" method="post" enctype="multipart/form-data">
            <label for="add" class=" btn bg-zinc-300"><i data-lucide="plus"></i>&nbsp;Add Content</label>
            <input type="checkbox" id="add" class="modal-toggle" />
            <div class="modal" role="dialog">
                <div class="modal-box max-w-2xl">
                    <h3 class="font-bold text-lg text-center">Add Content</h3>
                    ...
                    <div>
                        ...
                        <div>
                            <div class=" form-control">
                                ...
                                <textarea class="textarea textarea-bordered" name="des" placeholder="Schedule here"></textarea>
                            </div>
                        </div>
                    </div>
            ...
            <div class="modal" role="dialog">
                <div class="modal-box">
                    ...
                    <h3 class="font-bold text-lg text-center">Post Content?</h3>
                    <p class="py-4 text-center text-balance">Once posted, the content will be visible to all users.</p>
                    <div class="modal-action justify-center">
    ...
    <section class=" flex flex-col mt-4 gap-4">
        <?php
        if (session()->has('aboutSuc')) { ?>
            <div role="alert" class="alert alert-success">
                <svg xmlns="http://www.w3.org/2000/svg" class="stroke-current shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <span><?= session()->aboutSuc ?></span>
            </div>
        <?php }
        if (session()->has('aboutErr')) { ?>
            <div role="alert" class="alert alert-error">
                <svg xmlns="http://www.w3.org/2000/svg" class="stroke-current shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <span><?= session()->aboutErr ?></span>
            </div>
        <?php }
        ?>
        <?php foreach ($about as $item) { ?>
            <div class="w-full grid grid-cols-4 p-1 bg-zinc-200 rounded shadow-lg">
                ...
                <div class="col-span-1 flex justify-end items-center gap-2">
                    <form action="/admin/about/edit" method="post" id="editForm-<?= $item['id'] ?>" enctype="multipart/form-data">
                        ...
                        <div class="modal" role="dialog">
                            <div class="modal-box max-w-2xl">
                                <h3 class="font-bold text-lg text-center">Edit Content</h3>
                                    ...
                                    <div class=" grid grid-cols-3 gap-1">
                                        ...
                                        <div class=" col-span-2 form-control">
                                            <div class=" form-control">
                                                ...
                                                <input type="text" name="name" id="" class=" input input-bordered" value="<?= $item['title'] ?>">
                                            </div>
                                            <div class=" form-control">
                                                <label for="" class=" label">
                                                    <span class=" label-text">Upload Image</span>
                                                </label>
                                                <input type="file" name="upload" id="imageInput" class=" file-input file-input-bordered" accept="image/*" value="<?= $item['img'] ?>" >
                                            </div>
                                        </div>
                                    </div>
                                    <div>
                                        <div class=" form-control">
                                            ...
                                            <textarea class="textarea textarea-bordered" name="des" placeholder="Description here"> <?= $item['des'] ?></textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="modal-action justify-center">
                    ...
                    <div class="modal" role="dialog">
                        <div class="modal-box">
                            ...
                            <p class="py-4 text-center text-balance">This will delete the services permanently. You cannot undo
                                this action.</p>
                            <form method="post" action="/admin/about/delete" class="modal-action justify-center">
                                <input type="hidden" name="id" value="<?= $item['id'] ?>">
